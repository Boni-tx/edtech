...
export default function LoginPage({ dict }: { dict: LoginDict }) {
  // useSearchParams precisa estar dentro de um Suspense boundary no App Router
  return (
    <Suspense fallback={null}>
      <LoginPageContent dict={dict} />
    </Suspense>
  );
}

function LoginPageContent({ dict }: { dict: LoginDict }) {
  ...
  function traduzErro(message: string): string {
    const map: Record<string, string> = {
      "Invalid login credentials": dict.errorInvalidCredentials,
      "User already registered": dict.errorUserExists,
      "Email not confirmed": dict.errorEmailNotConfirmed,
      "Password should be at least 6 characters": dict.errorPasswordTooShort,
      "Email address is invalid": dict.errorInvalidEmail,
      "email rate limit exceeded": dict.errorRateLimit,
      over_email_send_rate_limit: dict.errorRateLimit,
    };
    if (map[message]) return map[message];
    if (/invalid/i.test(message)) return dict.errorInvalidEmail;
    if (/rate limit/i.test(message)) return dict.errorRateLimit;
  ...
  const handleEmailLogin = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (!EMAIL_REGEX.test(email)) errors.email = dict.errorEmailRequired;
    if (password.length < MIN_PASSWORD_LENGTH) {
      errors.password = dict.errorPasswordLength;
    }
  ...
  const handleEmailSignup = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (name.length < 2) errors.name = dict.errorNameRequired;
    if (!EMAIL_REGEX.test(email)) errors.email = dict.errorEmailRequired;
    if (password.length < MIN_PASSWORD_LENGTH) {
      errors.password = dict.errorPasswordLength;
    }
    if (confirmPassword !== password) errors.confirmPassword = dict.errorPasswordMismatch;
    ...
    if (!data.session) {
      setAuthNotice(dict.signupConfirmEmailNotice);
      return;
    }
        ...
        <Card className="overflow-hidden">
          <CardHeader className="items-center pt-8 text-center">
            <CardTitle className="text-2xl">
              {tab === "login" ? dict.welcomeBack : dict.createAccount}
            </CardTitle>
            <CardDescription>
              {tab === "login" ? dict.signInSubtitle : dict.signUpSubtitle}
            </CardDescription>
          </CardHeader>
          ...
          <CardContent className="pt-4">
            ...
            <div className="relative mb-7 grid grid-cols-2 rounded-full bg-navy-50 p-1">
              {(["login", "signup"] as Tab[]).map((t) => (
                ...
                >
                  ...
                  {t === "login" ? dict.tabLogin : dict.tabSignup}
                </button>
              ))}
            </div>
            ...
            {tab === "login" ? (
              <div className="space-y-5">
                ...
                <Divider label={dict.orWithEmail} />
              ...
              <div className="space-y-5">
                ...
                <div>
                  <Label className="mb-2.5">{dict.roleQuestion}</Label>
                  <div className="grid grid-cols-2 gap-3">
                    <RoleButton
                      active={role === "aluno"}
                      onClick={() => setRole("aluno")}
                      icon={<BookOpen className="h-4 w-4" />}
                      label={dict.roleStudent}
                    />
                    <RoleButton
                      active={role === "professor"}
                      onClick={() => setRole("professor")}
                      icon={<GraduationCap className="h-4 w-4" />}
                      label={dict.roleTeacher}
                    />
                  </div>
                </div>
                ...
                <Divider label={dict.or} />

                <form onSubmit={handleEmailSignup} noValidate className="space-y-4">
                  <div>
                    <Label htmlFor="signup-name">{dict.fullName}</Label>
                    <div className="relative">
                  ...
                  <div>
                    <Label htmlFor="signup-email">{dict.email}</Label>
                    <div className="relative">
                      <Mail className="pointer-events-none absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-navy-300" />
                      <Input
                        id="signup-email"
                  ...
                  <div>
                    <Label htmlFor="signup-password">{dict.password}</Label>
                    <div className="relative">
                      <Lock className="pointer-events-none absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-navy-300" />
                      <Input
                        id="signup-password"
                  ...
                  <div>
                    <Label htmlFor="signup-confirm-password">{dict.confirmPassword}</Label>
                    <div className="relative">
                      <Lock className="pointer-events-none absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-navy-300" />
                      <Input
                  ...
                  <Button type="submit" size="lg" className="w-full" disabled={isSubmitting}>
                    {isSubmitting ? dict.creatingAccount : dict.createAccountButton}
                  </Button>
                </form>
              </div>
            )}
