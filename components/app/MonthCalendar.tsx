  ...
  return (
    <div>
      <div className="rounded-2xl border border-navy-900/8 bg-white p-6 shadow-card">
        <p className="mb-4 text-center text-sm font-semibold capitalize text-navy-900">{monthLabel}</p>

        <div className="grid grid-cols-7 gap-1 text-center text-xs font-medium text-navy-300">
          {WEEKDAYS.map((w, i) => (
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  lessons
                    ? "cursor-pointer bg-navy-900 font-semibold text-white hover:bg-navy-700"
        ...
        <div className="mt-4 space-y-2">
          {selectedLessons.map((lesson) => {
            ...
            return (
              <div
                key={lesson.id}
                className="flex items-center justify-between rounded-xl border border-navy-900/8 bg-white p-4 shadow-card"
              >
                <div>
                  <p className="text-sm font-semibold text-navy-900">{professor?.name}</p>
                  <p className="text-xs text-navy-500">{professor?.subject}</p>
                </div>
                <span className="text-sm font-medium text-navy-700">{time}</span>
              </div>
            );
          })}
        </div>
